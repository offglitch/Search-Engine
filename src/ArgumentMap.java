...
public class ArgumentMap {
	 ...
	 */
	public ArgumentMap() {
		// Initializing the HashMap as String, String
	...
	public void parse(String[] args) {
		for (int i = 0; i < args.length; i++) {
			String flagArg = args[i];
			String valueArg = null;
			boolean isFlag = isFlag(flagArg);
			if (i != args.length - 1) {
				valueArg = args[i + 1];
				if (!isValue(valueArg)) {
					valueArg = null;
	...
	public static boolean isFlag(String arg) {
		...
		return arg.trim().length() >= 2 && arg.startsWith("-");
	}
	...
	public String getString(String flag, String defaultValue) {
		String value = map.getOrDefault(flag, null);
		return value == null ? defaultValue : value;
	}

	public int getValue(String flag, int defaultValue) {
		if (map.get(flag) != null) {
			if (isInt(map.get(flag))) {
